...
async function verificarValidacionesFechas(token, publicacionId) {
  ...
  if (respuestaPasado.statusCode === 201) {
    problemasEncontrados.push('❌ PROBLEMA: Se acepta fecha de inicio en el pasado');
  ...
  if (respuestaInvertida.statusCode === 201) {
    problemasEncontrados.push('❌ PROBLEMA: Se acepta fecha de fin anterior a fecha de inicio');
    console.log('   ❌ PROBLEMA: Se acepta fecha de fin anterior a fecha de inicio');
  } else {
    console.log('   ✅ Correctamente rechazada');
  }
  ...
  if (respuestaFormato.statusCode === 201) {
    problemasEncontrados.push('❌ PROBLEMA: Se acepta formato de fecha inválido');
...
async function verificarValidacionesPublicacion(token) {
  ...
  const respuestaInexistente = await crearReserva(token, reservaPublicacionInexistente);
  if (respuestaInexistente.statusCode === 201) {
    problemasEncontrados.push('❌ PROBLEMA: Se acepta reserva para publicación inexistente');
    console.log('   ❌ PROBLEMA: Se acepta reserva para publicación inexistente');
  } else {
    console.log('   ✅ Correctamente rechazada');
  }
...
async function verificarValidacionesCampos(token) {
  ...
  const respuestaSinPublicacion = await crearReserva(token, reservaSinPublicacion);
  if (respuestaSinPublicacion.statusCode === 201) {
    problemasEncontrados.push('❌ PROBLEMA: Se acepta reserva sin publicacionId');
  ...
  const respuestaSinFechaInicio = await crearReserva(token, reservaSinFechaInicio);
...
async function main() {
  ...
  try {
    ...
    console.log(`📦 Usando publicación: ${publicaciones[0].titulo} (${publicacionId})`);
    
    // Verificar validaciones
    const problemasFechas = await verificarValidacionesFechas(token, publicacionId);
    ...
    const todosLosProblemas = [...problemasFechas, ...problemasPublicacion, ...problemasCampos];
    ...
    if (todosLosProblemas.length > 0) {
      ...
      console.log('3. Asegurar que todos los campos obligatorios sean validados');
