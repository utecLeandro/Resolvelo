...
async function obtenerPublicaciones(token) {
  ...
  try {
    ...
    if (response.status === 200) {
      const publicaciones =
        response.data.publicaciones || response.data.data || response.data;
      const paginacion = response.data.paginacion || {};
      const total = paginacion.totalElementos || publicaciones.length;
      console.log(
        `✅ Se encontraron ${publicaciones.length} publicaciones (Total: ${total})`,
      );
...
async function obtenerMisReservas(token) {
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      const count = Array.isArray(reservas)
        ? reservas.length
...
async function main() {
  ...
  if (!usuario) {
    console.log(
      "❌ No se pudo obtener el perfil del usuario. Terminando pruebas.",
    );
    return;
  }
  ...
  console.log(
    `📝 Usando publicación: ${primeraPublicacion.titulo} (ID: ${primeraPublicacion.id})`,
  );
