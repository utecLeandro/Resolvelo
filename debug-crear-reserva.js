...
async function obtenerPublicaciones(token) {
  ...
  if (response.statusCode === 200) {
    ...
    console.log('❌ Error obteniendo publicaciones:', response.statusCode, response.body);
    return [];
  }
}
...
async function obtenerUsuarios(token) {
  ...
  if (response.statusCode === 200) {
    ...
    console.log('❌ Error obteniendo usuarios:', response.statusCode, response.body);
    return [];
  }
}

async function crearReserva(token, reservaData) {
  const response = await makeRequest('POST', '/api/usuarios/reservas/crear', reservaData, token);
  return response;
}
...
async function main() {
  try {
    ...
    console.log(`📦 Usando publicación: ${publicacion.titulo} (${publicacion.id})`);
    ...
    const usuariosIds = ['cmgv48937000c133iilmqhd1m', 'cmgv48937000e133iilmqhd1p', 'cmgv48937000f133iilmqhd1q'];
