...
async function registrarUsuario(userData) {
  try {
    ...
    console.error(
      `❌ Error al registrar ${userData.email}:`,
...
async function crearPublicacion(token, publicacionData) {
  try {
    ...
    console.log(
      `✅ Publicación "${publicacionData.titulo}" creada exitosamente`,
    );
    return response.data;
  } catch (error) {
    console.error(
...
async function main() {
  ...
  const publicacion = await crearPublicacion(tokenFederico, {
    ...
    descripcion:
      "Excelente guitarra acústica en perfecto estado, ideal para principiantes y profesionales",
    categoria: "GUITARRAS",
    marca: "Yamaha",
    modelo: "FG830",
  ...
  const reserva = await crearReserva(tokenMaria, {
    ...
    precioTotal: 100.0, // 4 días * 25/día
    comisionPlataforma: 10.0, // 10% de comisión
    tipoEntrega: "RETIRO",
  ...
  if (reserva) {
    ...
    console.log(
      `   - Federico (propietario): federico@test.com / Federico123!`,
    );
    console.log(`   - María (arrendataria): maria@test.com / Maria123!`);
    console.log(
      `   - Publicación ID: ${publicacion.data?.id || publicacion.id}`,
    );
