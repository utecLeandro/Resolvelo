  ...
  >
    ...
    <div class="flex items-center justify-between mb-6">
      <div>
        ...
        <p class="text-sm text-gray-600">
          Administra el estado y verificación de las cuentas
        </p>
      </div>
    ...
    <div class="bg-white border border-gray-200 rounded-lg shadow-sm p-4 mb-6">
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1" for="rol"
            >Rol</label
          >
          <select
            id="rol"
    ...
    <div
      v-if="!cargando && !errorMensaje && (!usuarios || usuarios.length === 0)"
    ...
    >
      ...
      >
        <div class="text-sm text-gray-600">
          ...
          {{ paginacion.totalElementos }} usuarios
        </div>
        <div class="flex items-center gap-2">
          <button
...
async function cargarUsuarios(pagina = 1) {
  ...
  try {
    ...
    const res: RespuestaUsuariosAdmin =
      await adminService.listarUsuarios(params);
    ...
    if (status === 401) {
      errorMensaje.value =
        "No estás autenticado. Por favor, inicia sesión e intenta nuevamente.";
    } else if (status === 403) {
      errorMensaje.value =
      ...
      errorMensaje.value =
        backendMsg || "Ocurrió un error al cargar los usuarios.";
    }
  } finally {
...
async function toggleEstado(u: UsuarioAdminListItem) {
  try {
    const res = await adminService.cambiarEstadoUsuario(
      u.id,
      !u.activo,
      !u.activo ? "Rehabilitación de cuenta" : "Deshabilitación",
    );
    ...
    if (status === 401) {
      ...
      errorMensaje.value =
        backendMsg || "Ocurrió un error al cambiar el estado del usuario.";
    }
  }
}
...
async function verificar(u: UsuarioAdminListItem) {
  try {
    const res = await adminService.verificarUsuario(
      u.id,
    ...
    if (status === 401) {
      ...
      errorMensaje.value =
        backendMsg || "Ocurrió un error al verificar al usuario.";
    }
  }
}
