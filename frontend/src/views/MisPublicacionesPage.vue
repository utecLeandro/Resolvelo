    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="mb-8">
        ...
        <p class="mt-2 text-gray-600">
          Gestiona tus instrumentos musicales y solicitudes de alquiler
        </p>
      </div>
        ...
        <div class="border-b border-gray-200">
          <nav class="-mb-px flex space-x-8" aria-label="Tabs">
            ...
            >
              Solicitudes Activas
              <span
                v-if="reservasActivasFiltradas.length > 0"
      ...
      <div v-if="pestanaActiva === 'publicaciones'">
        ...
        <div v-else-if="publicaciones.length === 0" class="text-center py-12">
          ...
          >
            <path
              ...
              stroke-width="2"
              d="M9 19V6l12-3v13M9 19c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3-2-1.343-2-3-2zm12-3c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zM9 10l12-3"
            />
          </svg>
          ...
          <p class="mt-1 text-sm text-gray-500">
            Comienza publicando tu primer instrumento musical.
          </p>
          <div class="mt-6">
            <router-link
        ...
        >
          ...
          >
            <!-- Imagen placeholder -->
            <div
              class="aspect-w-16 aspect-h-9 bg-gradient-to-br from-blue-50 to-indigo-100"
            >
              <div class="w-full h-48 flex items-center justify-center">
                <svg
                  class="h-12 w-12 text-blue-400 opacity-60"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                ...
                <div class="flex-1">
                  ...
                  <div class="flex items-center space-x-2 mb-3">
                    ...
                    <span
                      ...
                      :class="{
                        'bg-green-100 text-green-800':
                          publicacion.estadoModeracion === 'APROBADA',
                        'bg-yellow-100 text-yellow-800':
                          publicacion.estadoModeracion === 'PENDIENTE_REVISION',
                        'bg-red-100 text-red-800':
                          publicacion.estadoModeracion === 'RECHAZADA',
                      }"
                    >
                      {{
                    ...
                    <div class="grid grid-cols-2 gap-2 text-xs">
                      ...
                      >
                        ...
                        <span class="font-medium text-gray-900">{{
                          publicacion.estadisticasReservas.total
                        }}</span>
                      </div>

                      <!-- Pendientes -->
                      <div
                        v-if="publicacion.estadisticasReservas.pendientes > 0"
                        class="flex items-center justify-between bg-yellow-50 px-2 py-1 rounded"
                      >
                        <span class="text-yellow-700">Pendientes:</span>
                        <span class="font-medium text-yellow-800">{{
                          publicacion.estadisticasReservas.pendientes
                        }}</span>
                      </div>

                      <!-- Aprobadas -->
                      <div
                        v-if="publicacion.estadisticasReservas.aprobadas > 0"
                        class="flex items-center justify-between bg-blue-50 px-2 py-1 rounded"
                      >
                        <span class="text-blue-700">Aprobadas:</span>
                        <span class="font-medium text-blue-800">{{
                          publicacion.estadisticasReservas.aprobadas
                        }}</span>
                      </div>

                      <!-- Confirmadas -->
                      <div
                        v-if="publicacion.estadisticasReservas.confirmadas > 0"
                        class="flex items-center justify-between bg-green-50 px-2 py-1 rounded"
                      >
                        <span class="text-green-700">Confirmadas:</span>
                        <span class="font-medium text-green-800">{{
                          publicacion.estadisticasReservas.confirmadas
                        }}</span>
                      </div>

                      <!-- Activas -->
                      <div
                        v-if="publicacion.estadisticasReservas.activas > 0"
                        class="flex items-center justify-between bg-purple-50 px-2 py-1 rounded"
                      >
                        <span class="text-purple-700">Activas:</span>
                        <span class="font-medium text-purple-800">{{
                          publicacion.estadisticasReservas.activas
                        }}</span>
                      </div>

                      <!-- Completadas -->
                      <div
                        v-if="publicacion.estadisticasReservas.completadas > 0"
                        class="flex items-center justify-between bg-emerald-50 px-2 py-1 rounded"
                      >
                        <span class="text-emerald-700">Completadas:</span>
                        <span class="font-medium text-emerald-800">{{
                          publicacion.estadisticasReservas.completadas
                        }}</span>
                      </div>

                      <!-- Rechazadas -->
                      <div
                        v-if="publicacion.estadisticasReservas.rechazadas > 0"
                        class="flex items-center justify-between bg-red-50 px-2 py-1 rounded"
                      >
                        <span class="text-red-700">Rechazadas:</span>
                        <span class="font-medium text-red-800">{{
                          publicacion.estadisticasReservas.rechazadas
                        }}</span>
                      </div>

                      <!-- Canceladas -->
                      <div
                        v-if="publicacion.estadisticasReservas.canceladas > 0"
                        class="flex items-center justify-between bg-gray-50 px-2 py-1 rounded"
                      >
                        <span class="text-gray-700">Canceladas:</span>
                        <span class="font-medium text-gray-800">{{
                          publicacion.estadisticasReservas.canceladas
                        }}</span>
                      </div>
                    </div>
                  </div>
      ...
      <div v-else-if="pestanaActiva === 'solicitudes'">
        ...
        <!-- Sin solicitudes -->
        <div
          v-else-if="solicitudesFiltradas.length === 0"
        ...
        >
          ...
          <h3 class="mt-2 text-sm font-medium text-gray-900">
            No hay solicitudes {{ subPestanaSolicitudes }}
          </h3>
          <p class="mt-1 text-sm text-gray-500">
            <span v-if="subPestanaSolicitudes === 'pendientes'"
              >Cuando recibas solicitudes de alquiler aparecerán aquí.</span
            >
            <span v-else-if="subPestanaSolicitudes === 'aprobadas'"
              >Las solicitudes que apruebes aparecerán aquí.</span
            >
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-start justify-between">
                    <div class="flex-1">
                        ...
                        >
                          ...
                          >
                            ...
                            <path
                              ...
                              stroke-linejoin="round"
                              stroke-width="2"
                              d="M12 14c1.657 0 3-1.343 3-3s-1.343-3-3-3-3 1.343-3 3 1.343 3 3 3z"
                            />
                          </svg>
                      ...
                      <div
                        v-if="solicitud.estado === 'CONFIRMADA'"
                        class="space-y-3"
                      >
                        <!-- Estado confirmada -->
                        <div
                      ...
                      <div
                        v-else-if="solicitud.estado === 'PENDIENTE'"
                      ...
                      >
                        ...
                        >
                          <span
                            v-if="procesandoSolicitud === solicitud.id"
                      ...
                      <div
                        v-else-if="solicitud.estado === 'RECHAZADA'"
      ...
      <div v-else-if="pestanaActiva === 'reservas-activas'">
        ...
        <div class="flex justify-between items-center mb-6">
          ...
          >
            <svg
              class="h-4 w-4 mr-1"
              :class="{ 'animate-spin': cargandoReservasActivas }"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
        ...
        <p class="text-sm text-gray-600 mb-4">
          Se muestran reservas en curso con pago exitoso. No incluye
          confirmadas.
        </p>

        <!-- Estado de carga reservas activas -->
        <div
          v-if="cargandoReservasActivas"
          class="flex justify-center items-center py-12"
        >
          <div
            class="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600"
        ...
        <div
          v-else-if="errorReservasActivas"
          class="bg-red-50 border border-red-200 rounded-md p-4 mb-6"
        >
          <div class="flex items-start">
            ...
            <div class="ml-3 flex-1">
              <h3 class="text-sm font-medium text-red-800">
                Error al cargar reservas activas
              </h3>
              <p class="mt-1 text-sm text-red-700">
                {{ errorReservasActivas }}
              </p>
              <div class="mt-3">
                <button
                  @click="cargarReservasActivas"
        ...
        <div
          v-else-if="reservasActivasFiltradas.length === 0"
          class="text-center py-12"
        >
          <svg
            class="mx-auto h-12 w-12 text-gray-400"
          ...
          <p class="mt-1 text-sm text-gray-500">
            Las reservas pagadas y en curso aparecerán aquí.
          </p>
        </div>
          ...
          >
            <div class="p-6">
              <div class="flex items-start space-x-4">
                ...
                <div class="flex-shrink-0">
                  <div
                    class="w-20 h-20 bg-gradient-to-br from-green-50 to-emerald-100 rounded-lg flex items-center justify-center"
                  >
                    <svg
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-start justify-between">
                    <div class="flex-1">
                      ...
                      <p class="text-sm text-gray-600 mb-2">
                        {{ reserva.publicacion.descripcion }}
                      </p>
      ...
      <div v-else-if="pestanaActiva === 'historial'">
        ...
        <div class="flex justify-between items-center mb-6">
          ...
          >
            <svg
              class="h-4 w-4 mr-1"
              :class="{ 'animate-spin': cargandoHistorial }"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
        ...
        <!-- Sin historial -->
        <div
          v-else-if="historialReservas.length === 0"
          class="text-center py-12"
        >
          <svg
            class="mx-auto h-12 w-12 text-gray-400"
          ...
          >
            <div class="p-6">
              <div class="flex items-start space-x-4">
                ...
                <div class="flex-shrink-0">
                  <div
                    class="w-20 h-20 bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg flex items-center justify-center"
                  >
                    <svg
                        ...
                        <div class="flex items-center space-x-2">
                          <div class="flex items-center">
                            <svg
                              ...
                              class="h-4 w-4"
                              :class="
                                i <= reserva.calificacion.puntuacion
                                  ? 'text-yellow-400'
                                  : 'text-gray-300'
                              "
                              fill="currentColor"
                          ...
                          <span class="text-sm text-gray-600"
                            >{{ reserva.calificacion.puntuacion }}/5</span
                          >
                        </div>
                        <p
                          v-if="reserva.calificacion.comentario"
      ...
      <div class="mt-3">
        ...
        <div v-if="reservaAcalificar" class="mb-4 p-3 bg-gray-50 rounded-md">
          <p class="text-sm text-gray-600">
            ...
            {{ reservaAcalificar.usuario?.nombre }}
            {{ reservaAcalificar.usuario?.apellido }}
          </p>
          <p class="text-sm text-gray-600">
...
const pestanaActiva = ref<
  "publicaciones" | "solicitudes" | "reservas-activas" | "historial"
>("publicaciones");
const subPestanaPublicaciones = ref<
  "todas" | "activas" | "pausadas" | "revision"
>("todas");
const subPestanaSolicitudes = ref<"pendientes" | "aprobadas" | "rechazadas">(
  "pendientes",
);
...
// Computed
const contadorSolicitudes = computed(
  () => (solicitudesPendientes.value || []).length,
);

// Computed properties para publicaciones
const publicacionesActivas = computed(() =>
  (publicaciones.value || []).filter(
    (p) => p.estado === "ACTIVA" && p.estadoModeracion === "APROBADA",
  ),
);
const publicacionesPausadas = computed(() =>
  (publicaciones.value || []).filter((p) => p.estado === "PAUSADA"),
);
const publicacionesEnRevision = computed(() =>
  (publicaciones.value || []).filter(
    (p) => p.estadoModeracion === "PENDIENTE_REVISION",
  ),
);

// Computed properties para solicitudes
const solicitudesPendientesCount = computed(() =>
  (solicitudesPendientes.value || []).filter((s) => s.estado === "PENDIENTE"),
);
const solicitudesAprobadas = computed(() =>
  (solicitudesPendientes.value || []).filter((s) => s.estado === "CONFIRMADA"),
);
const solicitudesRechazadas = computed(() =>
  (solicitudesPendientes.value || []).filter((s) => s.estado === "RECHAZADA"),
);
...
const reservasActivasFiltradas = computed(() => {
  ...
  return rs.filter(
    (r) =>
      r.estado === "EN_CURSO" &&
      (r.transacciones || []).some((tx: any) => tx.estado === "COMPLETADA"),
  );
});
...
onMounted(async () => {
  ...
  console.log(
    "🔍 Estado inicial procesandoSolicitud:",
    procesandoSolicitud.value,
  );
  await cargarPublicaciones();
  await cargarSolicitudesPendientes();
  window.addEventListener(
    "reserva-actualizada",
    manejarActualizacionPropietario as EventListener,
  );
  ...
  try {
    ...
    bc.onmessage = async (ev: MessageEvent) => {
      const data = ev.data || {};
      if (
        data?.tipo === "reserva-actualizada" &&
        (data?.nuevoEstado === "EN_CURSO" || data?.accion === "pago-aprobado")
      ) {
...
const cambiarPestana = (
  pestana: "publicaciones" | "solicitudes" | "reservas-activas" | "historial",
) => {
  pestanaActiva.value = pestana;
  if (
    pestana === "solicitudes" &&
    (!solicitudesPendientes.value || solicitudesPendientes.value.length === 0)
  ) {
    cargarSolicitudesPendientes();
  } else if (
    pestana === "reservas-activas" &&
    (!reservasActivas.value || reservasActivas.value.length === 0)
  ) {
    cargarReservasActivas();
  } else if (
    pestana === "historial" &&
    (!historialReservas.value || historialReservas.value.length === 0)
  ) {
...
const aprobarSolicitud = async (solicitud: SolicitudReserva) => {
  ...
  try {
    ...
    if (response.success) {
      ...
      const solicitudIndex = solicitudesPendientes.value.findIndex(
        (s) => s.id === solicitud.id,
      );
      if (solicitudIndex !== -1) {
...
const confirmarRechazo = async () => {
  ...
  try {
    const response = await reservasService.rechazarReserva(
    ...
    if (response.success) {
      // Actualizar el estado local inmediatamente
      const solicitudIndex = solicitudesPendientes.value.findIndex(
        (s) => s.id === solicitudArechazar.value!.id,
      );
      if (solicitudIndex !== -1) {
...
const cargarReservasActivas = async () => {
  try {
    ...
    if (response.success) {
      ...
      console.log(
        "✅ Reservas activas cargadas:",
        reservasActivas.value.length,
      );
    } else {
    ...
    errorReservasActivas.value =
      err.message || "Error al cargar las reservas activas";
...
const cargarHistorialReservas = async () => {
  try {
    ...
    const response =
      await reservasService.obtenerHistorialReservasPropietario();

    if (response.success) {
      historialReservas.value = response.data || [];
      console.log(
        "✅ Historial de reservas cargado:",
        historialReservas.value.length,
      );
    } else {
      throw new Error(
        response.message || "Error al obtener historial de reservas",
      );
    }
  } catch (err: any) {
