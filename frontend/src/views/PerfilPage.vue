...
onMounted(async () => {
  ...
  if (!token) {
    error.value =
      "No estás autenticado. Inicia sesión para acceder a tu perfil.";
    return;
  }
  cargando.value = true;
  try {
    ...
    if (userDataRaw) {
      ...
      try {
        const perfil: UsuarioPerfil = await usuarioService.obtenerPerfil(
          userData.id,
        );
        nombre.value = perfil.nombre || "";
        apellido.value = perfil.apellido || "";
        ...
        error.value =
          innerErr?.response?.data?.message || "Error al cargar tu perfil.";
      }
    } else {
...
const guardarCambios = async () => {
  ...
  try {
    ...
    error.value =
      e?.response?.data?.message || "No se pudo actualizar el perfil.";
  } finally {
...
const cargarMisPublicaciones = async () => {
  ...
  try {
    misPublicaciones.value =
      await publicacionesService.obtenerMisPublicaciones();
  } catch (e: any) {
    errorPublicaciones.value =
      e?.response?.data?.message || "Error al cargar tus publicaciones.";
  } finally {
  ...
  <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div v-else>
      ...
      <div v-if="pestanaActiva === 'perfil'">
        ...
        >
          ...
          <div class="flex items-center gap-3">
            ...
            <span v-if="mensaje" class="text-green-600" aria-live="polite">{{
              mensaje
            }}</span>
            <span
              v-if="error"
      ...
      <div v-if="pestanaActiva === 'publicaciones'">
        <div v-if="cargandoPublicaciones" class="text-gray-600">
          Cargando publicaciones…
        </div>
        <div v-else-if="errorPublicaciones" class="text-red-600">
          {{ errorPublicaciones }}
        </div>
