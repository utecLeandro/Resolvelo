<template>
  <div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="bg-green-50 border border-green-200 rounded-md p-6">
        <div class="flex items-start">
          ...
          <div>
            ...
            <p class="mt-1 text-sm text-green-700">Tu pago fue aprobado. A continuación el resumen:</p>
          </div>
        </div>
      </div>
      ...
      <div v-else-if="transaccion" class="bg-white rounded-lg shadow-md p-6 mt-6">
        ...
        <div v-if="transaccion.reserva" class="mt-6">
          ...
          <p class="text-sm text-gray-700 mt-1"><strong>Instrumento:</strong> {{ transaccion.reserva.publicacion.titulo }}</p>
          <p class="text-sm text-gray-700"><strong>Precio por día:</strong> ${{ transaccion.reserva.publicacion.precioPorDia }}</p>
          <p class="text-sm text-gray-700"><strong>Fecha inicio:</strong> {{ formatearFecha(transaccion.reserva.fechaInicio) }}</p>
          <p class="text-sm text-gray-700"><strong>Fecha fin:</strong> {{ formatearFecha(transaccion.reserva.fechaFin) }}</p>
        </div>

        <div class="mt-6 space-y-3">
          <div class="flex items-center text-sm text-gray-600">
            <svg class="h-4 w-4 text-gray-400 mr-2" viewBox="0 0 24 24" fill="none" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <span>Serás redirigido a <strong>Mis reservas</strong> en {{ redirectCountdown }} segundos…</span>
          </div>
          <div class="flex gap-3">
            <router-link to="/mis-reservas" class="bg-green-600 text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-green-700 transition-colors duration-200">
              Ir ahora
            </router-link>
            <button type="button" class="px-4 py-2 rounded-md text-sm font-medium border border-gray-300 text-gray-700 hover:bg-gray-50" @click="cancelarAutoRedirect">
...
const formatearFecha = (fecha: string) => new Date(fecha).toLocaleDateString('es-UY', { year: 'numeric', month: 'long', day: 'numeric' })
