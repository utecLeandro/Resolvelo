...
const ordenSeleccionado = ref<"relevancia" | "precio_asc" | "precio_desc">(
  "relevancia",
);
...
const tituloSeccion = computed(() => {
  ...
  if (filtros.value.categoria) {
    ...
    const base =
      categorias[filtros.value.categoria] || "Instrumentos musicales";
    if (tieneFechas) {
...
const cargarPublicaciones = async () => {
  try {
    ...
    if (filtros.value.fechaInicio && filtros.value.fechaFin) {
      ...
      respuesta = await publicacionesService.obtenerPublicaciones(
        filtros.value,
      );
    }
    ...
    error.value =
      "No se pudieron cargar los instrumentos. Por favor, intenta nuevamente.";
  } finally {
...
function obtenerIdOrdenDesdeFiltros(
  f: FiltrosPublicacion,
): "relevancia" | "precio_asc" | "precio_desc" {
...
watch(
  ...
  (newQuery) => {
    ...
    filtros.value = {
      ...
      precioMinimo: newQuery.precioMinimo
        ? Number(newQuery.precioMinimo)
        : undefined,
      precioMaximo: newQuery.precioMaximo
        ? Number(newQuery.precioMaximo)
        : undefined,
