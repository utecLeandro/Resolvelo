    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div v-if="cargando" class="flex justify-center items-center py-12">
        ...
        <span class="ml-3 text-gray-600"
          >Cargando información de la reserva...</span
        >
      </div>
      ...
      <div v-else-if="reserva" class="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div class="space-y-4">
            ...
            <div class="border-t pt-4">
              <div class="flex justify-between text-sm">
                ...
                <span class="font-medium"
                  >${{ reserva.publicacion.precioPorDia }}</span
                >
              </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
          ...
          <div
            v-if="brickError"
            class="mt-4 bg-red-50 border border-red-200 rounded-md p-3 text-sm text-red-700"
          >
            {{ brickError }}
          </div>
        </div>
...
const cargarReserva = async () => {
  try {
    ...
    if (
      ...
      reservaEncontrada.estado !== "EN_CURSO"
    ) {
...
const renderPaymentBrick = async () => {
  try {
    ...
    const mp = new (window as any).MercadoPago(mpPublicKey.value, {
      locale: "es-UY",
    });
      ...
      callbacks: {
        ...
        onSubmit: ({ selectedPaymentMethod, formData }: any) => {
          return new Promise<void>((resolve, reject) => {
            transaccionesService
              .procesarPagoBrick({
                formData: { ...formData, transaction_amount: calcularTotal() },
                transaccionId: transaccionIdMp.value,
                preferenceId: preferenciaId.value,
              })
              .then(async (tx) => {
                if (tx?.estado === "COMPLETADA") {
                  ...
                  router.replace({
                    path: "/pago-exitoso",
                    query: {
                      external_reference: transaccionIdMp.value,
                      status: "pending",
                    },
                  });
                } else {
                  router.replace({
                    path: "/pago-error",
                    query: { status: (tx?.estado || "error").toLowerCase() },
                  });
                }
                resolve();
              })
...
onMounted(async () => {
  ...
  try {
    const pref = await transaccionesService.crearPreferenciaMercadoPago(
      reserva.value.id,
      formPago.value.descripcion ||
        `Pago de reserva para ${reserva.value.publicacion.titulo}`,
    );
    if (!pref || pref.ok !== true) {
      throw new Error(
        pref?.message || "No se pudo crear la preferencia de pago",
      );
    }
...
const finalizarFlujoAprobado = async (transaccionId: string) => {
  try {
    ...
    const reservaId =
      tx?.reserva?.id || localStorage.getItem("ultimoPago_reservaId") || "";
    try {
    ...
    router.replace({
      path: "/pago-exitoso",
      query: { external_reference: transaccionId, status: "approved" },
    });
  } catch (e: any) {
    console.warn("finalizarFlujoAprobado error:", e);
    router.replace({
      path: "/pago-exitoso",
      query: { external_reference: transaccionId },
    });
  }
};
</script>
