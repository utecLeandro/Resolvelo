...
export const transaccionesService = {
  ...
  async confirmarPagoMercadoPago(paymentId: string): Promise<Transaccion> {
    const response = await api.post("/transacciones/mercado-pago/confirmar", {
      paymentId,
    });
    return response.data;
  },

  async verificarEstadoMercadoPagoPorTransaccion(
    id: string,
  ): Promise<Transaccion> {
    const response = await api.get(
      `/transacciones/mercado-pago/verificar/${id}`,
    );
    return response.data;
  },
};
