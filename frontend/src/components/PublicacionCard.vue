<template>
  ...
  >
    ...
    <div
      class="relative aspect-square overflow-hidden rounded-t-xl bg-gradient-to-br from-blue-50 to-indigo-100"
    >
      <!-- Icono musical -->
      <div
      ...
      >
        <svg
          ...
          fill="currentColor"
          viewBox="0 0 24 24"
          :aria-label="`Icono de ${publicacion.titulo}`"
        >
          <path
      ...
      <div class="absolute top-3 left-3">
        <span
          class="bg-white/90 backdrop-blur-sm text-gray-800 px-2 py-1 rounded-full text-xs font-medium"
        >
    ...
    <div class="p-4">
      ...
      <!-- Marca y modelo -->
      <div
        v-if="publicacion.marca || publicacion.modelo"
      ...
      <div class="mt-3 flex items-center justify-between text-xs text-gray-500">
        <span
          >Mín. {{ publicacion.diasMinimoAlquiler }} día{{
            publicacion.diasMinimoAlquiler > 1 ? "s" : ""
          }}</span
        >
...
const ubicacionCompleta = computed(() => {
  const partes = [
    props.publicacion.ciudad,
    props.publicacion.departamento,
  ].filter(Boolean);
  return partes.join(", ");
});
