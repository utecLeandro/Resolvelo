...
async function testEliminacionCompleta() {
  try {
    console.log('🧪 Iniciando prueba completa de eliminación de publicaciones...\n');
    ...
    const nuevaPublicacion = {
      ...
      descripcion: 'Guitarra acústica en excelente estado para pruebas de eliminación',
      categoria: 'GUITARRAS',
      precioPorDia: 25.00,
    ...
    const publicacionCreada = await makeRequest(`${BASE_URL}/publicaciones?usuarioId=${userId}`, {
      method: 'POST',
      headers: { 
    ...
    const publicacionesUsuario = await makeRequest(`${BASE_URL}/publicaciones/mis-publicaciones?usuarioId=${userId}`, {
      headers: { Authorization: `Bearer ${token}` }
    });

    const publicacionEncontrada = publicacionesUsuario.find(p => p.id === publicacionId);
    ...
    await makeRequest(`${BASE_URL}/publicaciones/${publicacionId}?usuarioId=${userId}`, {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${token}` }
    });
    ...
    const publicacionesActualizadas = await makeRequest(`${BASE_URL}/publicaciones/mis-publicaciones?usuarioId=${userId}`, {
      headers: { Authorization: `Bearer ${token}` }
    });

    const publicacionEliminada = publicacionesActualizadas.find(p => p.id === publicacionId);
    ...
    console.log('7. 🔒 Verificando que la publicación no es accesible directamente...');
    try {
    ...
    console.log('🎉 ¡Prueba completa exitosa! La funcionalidad de eliminación funciona correctamente.');
