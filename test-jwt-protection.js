...
async function testSinToken() {
  ...
  const rutasProtegidas = [
    {
      method: "GET",
      path: "/api/usuarios/reservas/listar",
    ...
    {
      method: "GET",
      path: "/api/usuarios/reservas/123",
    ...
    {
      method: "PATCH",
      path: "/api/usuarios/reservas/123/cancelar",
    ...
    {
      method: "GET",
      path: "/api/publicaciones/mis-publicaciones",
      description: "Mis publicaciones",
    },
    {
      method: "PATCH",
      path: "/api/publicaciones/123",
  ...
  for (const ruta of rutasProtegidas) {
    try {
      ...
      const response = await makeRequest(
        options,
        ruta.method === "POST" || ruta.method === "PATCH" ? {} : null,
      );

      if (response.statusCode === 401) {
        console.log(
          `✅ ${ruta.description}: Correctamente protegida (401 Unauthorized)`,
        );
      } else {
        console.log(
          `❌ ${ruta.description}: NO protegida (${response.statusCode})`,
        );
        console.log(`   Respuesta: ${JSON.stringify(response.body)}`);
      }
    } catch (error) {
      console.log(
        `⚠️  ${ruta.description}: Error en petición - ${error.message}`,
      );
    }
  }
}
...
async function testTokenInvalido() {
  ...
  for (const ruta of rutasProtegidas) {
    try {
      ...
      if (response.statusCode === 401) {
        console.log(
          `✅ ${ruta.description}: Token inválido rechazado correctamente (401)`,
        );
      } else {
        console.log(
          `❌ ${ruta.description}: Token inválido aceptado (${response.statusCode})`,
        );
        console.log(`   Respuesta: ${JSON.stringify(response.body)}`);
      }
    } catch (error) {
    ...
    for (const ruta of rutasProtegidas) {
      try {
        ...
        if (response.statusCode === 200) {
          console.log(
            `✅ ${ruta.description}: Acceso autorizado correctamente (200)`,
          );
        } else {
          console.log(
            `⚠️  ${ruta.description}: Respuesta inesperada (${response.statusCode})`,
          );
          console.log(`   Respuesta: ${JSON.stringify(response.body)}`);
        }
      } catch (error) {
