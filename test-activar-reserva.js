...
async function testActivarReserva() {
  try {
    ...
    if (!reservaConfirmada) {
      ...
      console.log(
        "Estados disponibles:",
        reservas.map((r) => `${r.id}: ${r.estado}`),
      );
      return;
    }

    console.log(
      `✅ Reserva encontrada: ${reservaConfirmada.id} (Estado: ${reservaConfirmada.estado})`,
    );
    ...
    const reservasActualizadas =
      reservaActualizadaResponse.data.data || reservaActualizadaResponse.data;
    const reservaActualizada = reservasActualizadas.find(
      (r) => r.id === reservaConfirmada.id,
    );
    if (reservaActualizada && reservaActualizada.estado === "EN_CURSO") {
      console.log("✅ Estado actualizado correctamente a EN_CURSO");
    } else {
      console.log(
        `❌ Estado no cambió. Estado actual: ${reservaActualizada?.estado || "No encontrada"}`,
      );
    }
