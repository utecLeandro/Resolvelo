...
async function login() {
  ...
  try {
    ...
    if (response.statusCode === 200) {
      ...
      throw new Error(
        `Login falló: ${response.statusCode} - ${JSON.stringify(response.body)}`,
      );
    }
  } catch (error) {
...
async function main() {
  ...
  try {
    ...
    if (response.statusCode === 201 || response.statusCode === 200) {
      ...
      console.log(
        "📋 ID:",
        response.body.data?.id || response.body.id || "No disponible",
      );
    } else {
    ...
    if (
      listResponse.body.publicaciones &&
      listResponse.body.publicaciones.length > 0
    ) {
