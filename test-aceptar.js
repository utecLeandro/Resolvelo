...
async function main() {
  ...
  if (solicitudesFederico.length > 0) {
    ...
    console.log(`7. Federico acepta la solicitud ${solicitudParaAceptar.id}...`);
    
    const resultado = await aceptarSolicitud(tokenFederico, solicitudParaAceptar.id);
    if (resultado) {
    ...
    const reservaAceptada = reservasMariaDepues.find(r => r.id === solicitudParaAceptar.id);
    ...
    const solicitudesFedericoDepues = await obtenerSolicitudesPendientes(tokenFederico);
    console.log(`📋 Federico tiene ${solicitudesFedericoDepues.length} solicitudes pendientes\n`);
  } else {
    console.log('ℹ️  No hay solicitudes pendientes para probar la funcionalidad de aceptar\n');
  }
