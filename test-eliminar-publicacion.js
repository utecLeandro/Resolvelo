...
async function testEliminarPublicacion() {
  try {
    ...
    if (publicaciones.length === 0) {
      console.log(
        "⚠️ No hay publicaciones para eliminar. Creando una publicación de prueba...",
      );
      ...
      const nuevaPublicacion = await makeRequest(
        `${API_BASE_URL}/publicaciones?usuarioId=${userId}`,
        {
          ...
          body: JSON.stringify({
            ...
            descripcion:
              "Esta es una publicación creada para probar la funcionalidad de eliminación",
            categoria: "GUITARRA",
            precioPorDia: 50,
    ...
    const publicacionEliminada = publicacionesFinales.find(
      (p) => p.id === publicacionAEliminar.id,
    );

    if (!publicacionEliminada) {
      console.log(
        "✅ Verificación exitosa: La publicación ya no aparece en la lista",
      );
    } else {
    ...
    try {
      await makeRequest(
        `${API_BASE_URL}/publicaciones/${publicacionAEliminar.id}`,
      );
      console.log("❌ Error: La publicación eliminada aún es accesible");
    } catch (error) {
      if (error.response && error.response.status === 404) {
        console.log(
          "✅ Verificación exitosa: La publicación eliminada no es accesible (404)",
        );
      } else {
