...
async function testMariaGarciaCompleto() {
  try {
    ...
    if (!perfilResponse.ok) {
      throw new Error(
        `Profile failed: ${perfilResponse.status} ${perfilResponse.statusText}`,
      );
    }
    ...
    const misPublicacionesResponse = await fetch(
      `${API_BASE_URL}/publicaciones/mis-publicaciones?usuarioId=${userId}`,
    );

    if (!misPublicacionesResponse.ok) {
      throw new Error(
        `Get publications failed: ${misPublicacionesResponse.status} ${misPublicacionesResponse.statusText}`,
      );
    }
    ...
    const nuevaPublicacion = {
      ...
      descripcion:
        "Micrófono dinámico profesional Shure SM58 en excelente estado",
      categoria: "AUDIO_PA",
      marca: "Shure",
      modelo: "SM58",
      precioPorDia: 25,
    ...
    if (!crearResponse.ok) {
      ...
      throw new Error(
        `Create publication failed: ${crearResponse.status} ${crearResponse.statusText} - ${errorText}`,
      );
    }
    ...
    const misPublicacionesFinalesResponse = await fetch(
      `${API_BASE_URL}/publicaciones/mis-publicaciones?usuarioId=${userId}`,
    );

    if (!misPublicacionesFinalesResponse.ok) {
      throw new Error(
        `Get final publications failed: ${misPublicacionesFinalesResponse.status} ${misPublicacionesFinalesResponse.statusText}`,
      );
    }

    const misPublicacionesFinalesData =
      await misPublicacionesFinalesResponse.json();
    console.log(
      `📋 María ahora tiene ${misPublicacionesFinalesData.length} publicaciones:`,
    );
