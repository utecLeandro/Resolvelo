...
async function testReservasActivasEHistorial() {
  try {
    ...
    try {
      ...
      console.log(
        `   📊 Reservas activas encontradas: ${reservasActivasResponse.data.count || 0}`,
      );

      if (
        reservasActivasResponse.data.data &&
        reservasActivasResponse.data.data.length > 0
      ) {
        ...
        reservasActivasResponse.data.data
          ...
          .forEach((reserva, index) => {
            console.log(
              `      ${index + 1}. ${reserva.publicacion?.titulo} - Estado: ${reserva.estado}`,
            );
            console.log(
              `         Fechas: ${new Date(reserva.fechaInicio).toLocaleDateString()} - ${new Date(reserva.fechaFin).toLocaleDateString()}`,
            );
            console.log(
      ...
      console.log(
        "   ❌ Error en endpoint de reservas activas:",
    ...
    try {
      ...
      console.log(
        `   📊 Reservas en historial: ${historialResponse.data.count || 0}`,
      );

      if (
        historialResponse.data.data &&
        historialResponse.data.data.length > 0
      ) {
      ...
      console.log(
        "   ❌ Error en endpoint de historial:",
        error.response?.data?.message || error.message,
      );
    }
    ...
    console.log(
      `      - data: ${Array.isArray(reservasActivasResponse.data.data) ? "Array" : "No es array"}`,
    );
    console.log(`      - count: ${reservasActivasResponse.data.count}`);
    console.log(
      `      - timestamp: ${reservasActivasResponse.data.timestamp ? "Presente" : "Ausente"}`,
    );

    console.log("   ✅ Estructura de respuesta de historial:");
    console.log(`      - success: ${historialResponse.data.success}`);
    console.log(
      `      - data: ${Array.isArray(historialResponse.data.data) ? "Array" : "No es array"}`,
    );
    console.log(`      - count: ${historialResponse.data.count}`);
    console.log(
      `      - timestamp: ${historialResponse.data.timestamp ? "Presente" : "Ausente"}`,
    );

    console.log("\n🎉 === PRUEBA COMPLETADA EXITOSAMENTE ===");
  } catch (error) {
    console.error(
      "❌ Error general en la prueba:",
