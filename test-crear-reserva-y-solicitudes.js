...
async function testCrearReservaYSolicitudes() {
  ...
  try {
    ...
    console.log('📋 Publicaciones disponibles:', publicacionesData.publicaciones?.length || 0);

    if (!publicacionesData.publicaciones || publicacionesData.publicaciones.length === 0) {
      console.log('❌ No hay publicaciones disponibles');
      return;
    }

    // Usar la primera publicación de Juan
    const publicacion = publicacionesData.publicaciones.find(p => p.propietarioId === juanId);
    if (!publicacion) {
    ...
    const crearReservaResponse = await fetch('http://localhost:3000/api/usuarios/reservas/crear', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
    ...
    const loginJuanResponse = await fetch('http://localhost:3000/api/auth/login', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (misSolicitudesResponse.status === 200) {
      ...
      console.log('📋 Solicitudes encontradas:', misSolicitudesData.length || 0);
      if (misSolicitudesData.length > 0) {
        console.log('📝 Primera solicitud:', JSON.stringify(misSolicitudesData[0], null, 2));
      }
    } else {
