...
async function testAuth() {
  ...
  for (const usuario of usuarios) {
    try {
      ...
      if (!loginResponse.ok) {
        console.log(
          `❌ Login fallido para ${usuario.email}:`,
          loginData.message,
        );
        console.log("\n" + "=".repeat(80) + "\n");
        continue;
      ...
      const publicacionesResponse = await fetch(
        `${API_BASE_URL}/publicaciones/mis-publicaciones?usuarioId=${user.id}`,
        {
          headers: {
            Authorization: `Bearer ${access_token}`,
          },
      ...
      console.log(
        `📋 Mis publicaciones (${publicacionesData.length} encontradas):`,
      );
      publicacionesData.forEach((pub, index) => {
        console.log(
          `   ${index + 1}. ${pub.titulo} (ID: ${pub.id}, Propietario: ${pub.propietarioId})`,
        );
      });
