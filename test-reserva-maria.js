...
async function testReservaMaria() {
    try {
        ...
        const loginResponse = await axios.post(`${BASE_URL}/api/auth/login`, credenciales);
        ...
        const publicacionResponse = await axios.get(`${BASE_URL}/api/publicaciones/${publicacionId}`);
        ...
        if (usuarioId === propietarioId) {
            console.log('❌ El cliente es el propietario de esta publicación, no puede reservarla');
            return;
        }
        
        console.log('✅ El cliente no es el propietario, puede proceder con la reserva');
        ...
        const reservaResponse = await axios.post(`${BASE_URL}/api/usuarios/reservas/crear`, datosReserva, {
            headers: {
        ...
        console.log('🎉 ¡Reserva creada exitosamente con el cliente!', reservaResponse.data);
        ...
        if (error.response?.data?.message) {
            console.log('🔍 Mensaje de error específico:', error.response.data.message);
        }
    }
}
