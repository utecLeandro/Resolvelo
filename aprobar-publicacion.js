...
async function aprobarPublicaciones() {
  try {
    ...
    console.log(
      `📋 Encontradas ${publicacionesPendientes.length} publicaciones pendientes:`,
    );
    ...
    if (publicacionesPendientes.length > 0) {
      ...
      console.log(
        `✅ ${resultado.count} publicaciones aprobadas exitosamente!`,
      );
    } else {
    ...
    console.log(
      `📋 Total de publicaciones aprobadas: ${publicacionesAprobadas.length}`,
    );
