...
async function crearReservaConfirmada() {
  try {
    ...
    const publicaciones = publicacionesResponse.data.data || publicacionesResponse.data;
    ...
    console.log(`✅ Usando publicación: ${publicacion.id} - ${publicacion.titulo}`);
    ...
    const crearResponse = await axios.post(`${BASE_URL}/api/usuarios/reservas/crear`, reservaData, {
      headers: {
        'Authorization': `Bearer ${token}`,
    ...
    console.log('\n✅ Reserva creada exitosamente. ID:', reservaCreada.data?.id || reservaCreada.id);
    console.log('📝 Para completar la prueba, necesitas que el propietario acepte la reserva.');
