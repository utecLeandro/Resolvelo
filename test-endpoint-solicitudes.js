...
async function testEndpointSolicitudes() {
  try {
    ...
    console.log(
      "🔍 Probando endpoint /api/usuarios/reservas/todas-mis-solicitudes...",
    );
    const solicitudesResponse = await axios.get(
      `${BASE_URL}/api/usuarios/reservas/todas-mis-solicitudes`,
      {
        headers: {
    ...
    if (Array.isArray(solicitudesResponse.data)) {
      console.log(
        `📊 Total de solicitudes: ${solicitudesResponse.data.length}`,
      );
      ...
      solicitudesResponse.data.forEach((s) => {
        conteos[s.estado]++;
        console.log(
          `  - ${s.estado}: "${s.publicacion?.titulo || "Sin título"}" por ${s.usuario?.email || "Sin email"}`,
        );
      });
