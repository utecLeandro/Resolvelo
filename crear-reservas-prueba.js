...
async function crearReserva(token, publicacionId, fechaInicio, fechaFin) {
  try {
    const response = await axios.post(`${BASE_URL}/usuarios/reservas`, {
      publicacionId,
      fechaInicio,
      fechaFin,
...
async function aprobarReserva(token, reservaId) {
  try {
    const response = await axios.post(`${BASE_URL}/usuarios/reservas/${reservaId}/aceptar`, {}, {
      headers: { Authorization: `Bearer ${token}` }
    });
    return response.data;
  } catch (error) {
    console.error('Error al aprobar reserva:', error.response?.data || error.message);
    throw error;
  }
}
...
async function rechazarReserva(token, reservaId, motivo = 'Prueba de rechazo') {
  try {
    const response = await axios.post(`${BASE_URL}/usuarios/reservas/${reservaId}/rechazar`, {
      motivo
    }, {
      headers: { Authorization: `Bearer ${token}` }
    });
    ...
    console.error('Error al rechazar reserva:', error.response?.data || error.message);
    throw error;
  }
}
...
async function main() {
  try {
    ...
    const reserva1 = await crearReserva(tokenMaria, publicacion.id, 
    ...
    const reserva2 = await crearReserva(tokenMaria, publicacion.id, 
    ...
    const reserva3 = await crearReserva(tokenMaria, publicacion.id, 
    ...
    await rechazarReserva(tokenFederico, reserva2.id, 'Fechas no disponibles - prueba');
