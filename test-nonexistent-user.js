...
async function testGtbumpUser() {
  ...
  try {
    ...
    if (!loginSuccess) {
      ...
      console.log(
        "💡 Necesitamos verificar cuál es la contraseña correcta en el seed",
      );
      return;
    }
    ...
    const publicacionesResponse = await fetch(
      `${BASE_URL}/publicaciones/mis-publicaciones?usuarioId=${userData.id}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
    ...
    if (publicacionesResponse.ok) {
      ...
      publicacionesData.forEach((pub, index) => {
        console.log(
          `   ${index + 1}. ${pub.titulo} - $${pub.precioPorDia}/día`,
        );
      });
    } else {
    ...
    const createResponse = await fetch(
      `${BASE_URL}/publicaciones?usuarioId=${userData.id}`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        ...
        body: JSON.stringify({
          ...
          descripcion:
            "Guitarra eléctrica Fender Stratocaster en excelente estado",
          categoria: "GUITARRAS",
          marca: "Fender",
          modelo: "Stratocaster",
