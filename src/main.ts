...
async function bootstrap() {
  ...
  try {
    ...
    express.get('/api/debug/mp/verificar/:transaccionId', async (req: Request, res: Response) => {
      try {
    ...
    express.get('/api/transacciones/mercado-pago/verificar/:transaccionId', async (req: Request, res: Response) => {
      try {
        const transaccionesService = app.get(TransaccionesService);
        const resp = await transaccionesService.verificarEstadoMercadoPagoPorTransaccion(req.params.transaccionId);
        res.json(resp);
