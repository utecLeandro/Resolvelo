...
export class UsuariosController {
  ...
  async obtenerMisSolicitudes(@Request() req: any) {
    ...
    const resultado =
      await this.reservasService.obtenerSolicitudesPendientes(propietarioId);
  ...
  async obtenerMisReservas(@Request() req: any) {
    ...
    const resultado =
      await this.reservasService.obtenerReservasArrendatario(arrendatarioId);
  ...
  async crearReserva(@Body() data: CrearReservaDto, @Request() _req: any) {
    try {
  ...
  async subirAvatar(@Param('id') id: string, @Request() req: any) {
    ...
    if (!usuarioId || usuarioId !== id) {
      throw new ForbiddenException(
        'No autorizado para subir avatar de otro usuario',
      );
    }
  ...
  @UseGuards(JwtAuthGuard)
  async cancelarReserva(@Param('id') id: string, @Request() _req: any) {
  ...
  @UseGuards(JwtAuthGuard)
  async confirmarReserva(@Param('id') id: string, @Request() _req: any) {
  ...
  @UseGuards(JwtAuthGuard)
  async rechazarReserva(@Param('id') id: string, @Request() _req: any) {
  ...
  async obtenerMisReservasActivas(@Request() req: any) {
    ...
    console.log(
      '🔍 [USUARIOS-CONTROLLER] Obteniendo reservas activas para propietario:',
      propietarioId,
    );
    const resultado =
      await this.reservasService.obtenerReservasActivasPropietario(
        propietarioId,
      );
    return {
      ...resultado,
  ...
  async obtenerMiHistorialReservas(@Request() req: any) {
    const propietarioId = req.user.id;
    console.log(
      '🔍 [USUARIOS-CONTROLLER] Obteniendo historial de reservas para propietario:',
      propietarioId,
    );
    const resultado =
      await this.reservasService.obtenerHistorialReservasPropietario(
        propietarioId,
      );
    return {
      ...resultado,
  ...
  async obtenerTodasMisSolicitudes(@Request() req: any) {
    console.log(
      '🚀🚀🚀 [USUARIOS-CONTROLLER] obtenerTodasMisSolicitudes - MÉTODO EJECUTÁNDOSE 🚀🚀🚀',
    );
    console.log(
      '🎯 [USUARIOS-CONTROLLER] obtenerTodasMisSolicitudes - Iniciando',
    );
    console.log('🎯 [USUARIOS-CONTROLLER] Usuario autenticado:', req.user);
    const propietarioId = req.user.id;
    console.log(
      '🎯 [USUARIOS-CONTROLLER] PropietarioId extraído:',
      propietarioId,
    );
    ...
    try {
      ...
      console.log(
        '🎯 [USUARIOS-CONTROLLER] Resultado del servicio:',
        resultado,
      );
      return {
        ...resultado,
      ...
      console.error(
        '🎯 [USUARIOS-CONTROLLER] Error en obtenerTodasMisSolicitudes:',
        error,
      );
      throw error;
    }
  ...
  async activarReserva(@Param('id') reservaId: string, @Request() _req: any) {
    try {
      ...
      console.log(
        '✅ [USUARIOS-CONTROLLER] Reserva activada exitosamente:',
        resultado,
      );
      return resultado;
    } catch (error) {
      console.error(
        '❌ [USUARIOS-CONTROLLER] Error al activar reserva:',
        error,
      );
      throw error;
    }
  ...
  @Get('reservas/:id')
  @UseGuards(JwtAuthGuard)
  async obtenerReserva(@Param('id') id: string, @Request() _req: any) {
