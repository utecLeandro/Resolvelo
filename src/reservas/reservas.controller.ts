...
export class ReservasController {
  ...
  async aceptarSolicitud(@Param('id') reservaId: string, @Request() req: any) {
    try {
      ...
      if (reserva.data.propietarioId !== req.user.id) {
        throw new BadRequestException('No tienes permisos para aceptar esta solicitud');
      }
  ...
  async rechazarSolicitud(@Param('id') reservaId: string, @Request() req: any) {
    try {
      ...
      // Verificar que el usuario autenticado es el propietario
      if (reserva.data.propietarioId !== req.user.id) {
        throw new BadRequestException('No tienes permisos para rechazar esta solicitud');
      }
  ...
  async obtenerTodasMisSolicitudes(@Request() req: any) {
    const propietarioId = req.user.id;
    console.log('🔍 [CONTROLLER] Obteniendo todas las solicitudes para propietario:', propietarioId);
    return this.reservasService.obtenerTodasLasSolicitudes(propietarioId);
  }
}