...
export class NotificacionesService {
  ...
  emitir(usuarioId: string, data: any) {
    try { this.sse.next({ usuarioId, data }) } catch {}
  }
  ...
  async listar(usuarioId: string) {
    ...
    items.sort((a: any, b: any) => new Date(b.timestamp as any).getTime() - new Date(a.timestamp as any).getTime())
  ...
  async enviarEmail(usuarioId: string, asunto: string, html: string) {
    try {
  ...
  async emitirCambioEstado(reserva: any, anterior: string | null, actual: string) {
