...
async function probarFlujoReservas() {
  ...
  try {
    ...
    const publicaciones =
      responseData.publicaciones || responseData.data || responseData;
    ...
    console.log(
      `   ✅ Usando publicación: "${publicacionSeleccionada.titulo}" (ID: ${publicacionSeleccionada.id})`,
    );
    console.log(
      `   📍 Propietario ID: ${publicacionSeleccionada.propietarioId}`,
    );
    ...
    const datosReserva = {
      ...
      comisionPlataforma: Math.round(
        publicacionSeleccionada.precioPorDia * 2 * 0.1,
      ),
    ...
    console.log(
      ...
      JSON.stringify(reservaResponse.data, null, 2),
    );
    const reservaId =
      reservaResponse.data.id ||
      reservaResponse.data.reserva?.id ||
      "ID no disponible";
    console.log(`   ✅ Reserva creada exitosamente. ID: ${reservaId}`);
    console.log(
      `   📅 Fechas: ${datosReserva.fechaInicio} a ${datosReserva.fechaFin}`,
    );
    ...
    const solicitudes = Array.isArray(solicitudesResponse.data)
      ? solicitudesResponse.data
      : solicitudesResponse.data.data || [];
    console.log(
      `   📊 Total de solicitudes encontradas: ${solicitudes.length}`,
    );
    console.log(
      "   📋 Estructura de respuesta:",
      JSON.stringify(solicitudesResponse.data, null, 2),
    );
    ...
    if (solicitudCreada) {
      ...
      console.log(
        `      - Fechas: ${solicitudCreada.fechaInicio} a ${solicitudCreada.fechaFin}`,
      );
    ...
    const todasReservas = await axios.get(
      `${BASE_URL}/reservas/mis-solicitudes`,
      {
        headers: { Authorization: `Bearer ${token}` },
      },
    );
