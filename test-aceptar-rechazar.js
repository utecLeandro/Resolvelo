...
async function main() {
  ...
  console.log(`📋 Federico tiene ${solicitudesFederico.length} solicitudes pendientes`);
  ...
  if (solicitudesFederico.length > 0) {
    ...
    console.log(`5. Federico acepta la solicitud ${solicitudParaAceptar.id}...`);
    
    const resultadoAceptar = await aceptarSolicitud(tokenFederico, solicitudParaAceptar.id);
    if (resultadoAceptar) {
    ...
    if (reservasMariaDespues.length > 0) {
      ...
      const reservaModificada = reservasMariaDespues.find(r => r.id === solicitudParaAceptar.id);
      if (reservaModificada) {
        if (reservaModificada.estado === 'CONFIRMADA') {
          ...
          console.log(`⚠️  La reserva tiene estado ${reservaModificada.estado}, se esperaba CONFIRMADA`);
        }
      }
    }
    console.log('');
  ...
  console.log('7. Verificando solicitudes pendientes de Federico después de la acción...');
  const solicitudesFedericoDespues = await obtenerSolicitudesPendientes(tokenFederico);
  console.log(`📋 Federico tiene ${solicitudesFedericoDespues.length} solicitudes pendientes`);
