...
async function debugReservas() {
  try {
    ...
    const publicaciones = publicacionesData.publicaciones || publicacionesData.data || publicacionesData;
    ...
    const crearReservaResponse = await fetch(`${BASE_URL}/usuarios/reservas/crear`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
    ...
    console.log('📝 Headers de respuesta:', Object.fromEntries(crearReservaResponse.headers.entries()));

    const reservaCreada = await crearReservaResponse.json();
    console.log('📝 Respuesta completa:', JSON.stringify(reservaCreada, null, 2));
    ...
    if (crearReservaResponse.ok) {
      ...
      const listarResponse = await fetch(`${BASE_URL}/usuarios/reservas/listar`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`
        }
      ...
      console.log('📋 Respuesta de listar reservas:', JSON.stringify(reservasData, null, 2));
    } else {
