...
async function testEndpoint() {
  try {
    ...
    const response = await axios.get(
      "http://localhost:3000/api/imagenes/publicacion/cmgzlg2ll0005qzm8n1oy1pgy",
      {
        headers: {
  ...
  try {
    ...
    const response = await axios.post(
      "http://localhost:3000/api/imagenes/publicacion/cmgzlg2ll0005qzm8n1oy1pgy/multiples",
      {},
      {
        headers: {
          Authorization: `Bearer ${TOKEN}`,
    ...
    if (error.response?.status === 400) {
      console.log(
        "✅ La ruta existe pero falta el archivo (error 400 esperado)",
      );
    }
  }
}
