...
async function debugSolicitudesUsuario() {
  try {
    ...
    publicaciones.forEach(p => console.log(`  - "${p.titulo}" del propietario ${p.propietario.email} (${p.propietarioId})`));
    ...
    Object.entries(reservasPorPropietario).forEach(([email, data]) => {
      ...
      console.log(`  Total: ${data.total} | Pendientes: ${data.pendientes} | Confirmadas: ${data.confirmadas} | Rechazadas: ${data.rechazadas}`);
      console.log('  Detalle de reservas:');
      data.reservas.forEach(r => {
        console.log(`    - ${r.estado}: "${r.publicacion}" solicitada por ${r.usuario} (ID: ${r.id})`);
      });
    });
    ...
    if (federico) {
      ...
      const publicacionesFederico = publicaciones.filter(p => p.propietarioId === federico.id);
      console.log(`  Publicaciones: ${publicacionesFederico.length}`);
      publicacionesFederico.forEach(p => console.log(`    - "${p.titulo}"`));
      
      const solicitudesFederico = reservas.filter(r => r.propietarioId === federico.id);
