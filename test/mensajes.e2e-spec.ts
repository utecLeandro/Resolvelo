...
class FakePrismaService {
  ...
  mensaje = {
    count: async ({ where: { reservaId } }: any) => this.mensajes.filter(m => m.reservaId === reservaId).length,
    create: async ({ data }: any) => {
      const msg = { id: `msg_${Date.now()}`, fechaCreacion: new Date().toISOString(), leido: false, ...data }
      this.mensajes.push(msg)
      return msg
    },
    updateMany: async ({ where: { reservaId, receptorId, leido }, data: { leido: nuevoLeido, fechaLectura } }: any) => {
      const upd = this.mensajes.filter(m => m.reservaId === reservaId && m.receptorId === receptorId && m.leido === leido)
      upd.forEach(m => { m.leido = nuevoLeido; m.fechaLectura = fechaLectura })
      return { count: upd.length }
    },
    findMany: async ({ where: { reservaId }, orderBy }: any) => {
      const arr = this.mensajes.filter(m => m.reservaId === reservaId)
      if (orderBy?.fechaCreacion === 'asc') arr.sort((a, b) => a.fechaCreacion.localeCompare(b.fechaCreacion))
      if (orderBy?.fechaCreacion === 'desc') arr.sort((a, b) => b.fechaCreacion.localeCompare(a.fechaCreacion))
      return arr
    }
  }
  ...
  reserva = {
    findUnique: async ({ where: { id }, select, include }: any) => {
      ...
      if (select) {
        const picked: any = {}
        Object.keys(select).forEach(k => { if (select[k]) picked[k] = (r as any)[k] })
        return picked
      }
      return r
    }
  ...
  usuario = {
    findUnique: async ({ where: { id }, select }: any) => {
      ...
      if (select) {
        const picked: any = {}
        Object.keys(select).forEach(k => { if (select[k]) picked[k] = (u as any)[k] })
        return picked
      }
      return u
    }
...
describe('Mensajes - flujo de envío (E2E)', () => {
  let app: INestApplication
  let prisma: FakePrismaService

  beforeAll(async () => {
    const moduleRef = await Test.createTestingModule({ imports: [AppModule, MensajesModule] })
