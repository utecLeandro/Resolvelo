...
class FakePrismaService {
  ...
  usuario = {
    ...
    findUnique: async ({ where }: { where: { id?: string; email?: string } }) => {
      ...
      if (where.email) return this.usuarios.find(u => u.email === where.email) || null
      return null
    },
    findFirst: async ({ where }: { where: any }) => {
      if (!where) return this.usuarios[0] || null
      const keys = Object.keys(where)
      return this.usuarios.find(u => keys.every(k => u[k] === where[k])) || null
    },
...
describe('Auth - GubUy Simulado (E2E)', () => {
  ...
  it('Debería completar authorize -> token -> profile', async () => {
    ...
    const authorize = await request(app.getHttpServer())
      .get(`/api/auth/gubuy/authorize?redirect_uri=${encodeURIComponent(redirectUri)}&state=abc`)
      .expect(302)
  ...
  it('Debería retornar 403 si usuario no verificado', async () => {
    const redirectUri = 'http://localhost/callback'
    const authorize = await request(app.getHttpServer())
      .get(`/api/auth/gubuy/authorize?redirect_uri=${encodeURIComponent(redirectUri)}&state=xyz`)
      .expect(302)
