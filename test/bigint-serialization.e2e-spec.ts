...
import { INestApplication, ValidationPipe, NestInterceptor, ExecutionContext, CallHandler } from '@nestjs/common'
      ...
      return [
        {
          ...
          imagenes: [
            { id: BigInt(3001), url: 'https://cdn/img1.jpg', esPrincipal: true },
          ],
...
describe('Serialización BigInt en respuestas (E2E)', () => {
  ...
  it('GET /api/publicaciones convierte BigInt a string', async () => {
    const res = await request(app.getHttpServer()).get('/api/publicaciones').expect(200)
  ...
  it('GET /api/usuarios/:id convierte BigInt a string', async () => {
    const res = await request(app.getHttpServer()).get('/api/usuarios/9001').expect(200)
