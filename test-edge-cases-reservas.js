  ...
  for (const caso of casosInvalidos) {
    try {
      ...
      if (response.statusCode >= 400) {
        console.log(`✅ ${caso.descripcion}: Correctamente rechazada (${response.statusCode})`);
        if (response.body.message) {
          console.log(`   Mensaje: ${response.body.message}`);
        }
      } else {
        console.log(`❌ ${caso.descripcion}: Incorrectamente aceptada (${response.statusCode})`);
      }
    } catch (error) {
      console.log(`⚠️  ${caso.descripcion}: Error en petición - ${error.message}`);
    }
  }
}
...
async function testPublicacionInexistente(token) {
  ...
  try {
    ...
    if (response.statusCode === 404 || response.statusCode === 400) {
      console.log(`✅ Publicación inexistente: Correctamente rechazada (${response.statusCode})`);
      if (response.body.message) {
        console.log(`   Mensaje: ${response.body.message}`);
      }
    } else {
      console.log(`❌ Publicación inexistente: Incorrectamente aceptada (${response.statusCode})`);
    }
  } catch (error) {
    console.log(`⚠️  Publicación inexistente: Error en petición - ${error.message}`);
  }
}
  ...
  try {
    ...
    if (response1.statusCode === 201 || response1.statusCode === 200) {
      ...
      if (response2.statusCode >= 400) {
        console.log(`✅ Reserva duplicada: Correctamente rechazada (${response2.statusCode})`);
        if (response2.body.message) {
          console.log(`   Mensaje: ${response2.body.message}`);
        }
      } else {
        console.log(`❌ Reserva duplicada: Incorrectamente aceptada (${response2.statusCode})`);
      }
    } else {
      console.log(`⚠️  No se pudo crear la primera reserva para probar duplicación (${response1.statusCode})`);
    }
  } catch (error) {
  ...
  for (const operacion of operaciones) {
    try {
      ...
      if (response.statusCode === 404) {
        console.log(`✅ ${operacion.descripcion}: Correctamente rechazada (404)`);
      } else {
        console.log(`❌ ${operacion.descripcion}: Respuesta inesperada (${response.statusCode})`);
        if (response.body.message) {
          console.log(`   Mensaje: ${response.body.message}`);
        }
      }
    } catch (error) {
      console.log(`⚠️  ${operacion.descripcion}: Error en petición - ${error.message}`);
    }
  }
}
