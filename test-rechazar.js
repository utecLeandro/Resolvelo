...
async function main() {
  ...
  console.log(
    `📋 Federico tiene ${solicitudes.data.length} solicitudes pendientes`,
  );
  ...
  if (solicitudes.data.length > 0) {
    ...
    if (reservasDespues && reservasDespues.success) {
      ...
      const reservaRechazada = reservasDespues.data.find(
        (r) => r.id === reservaId,
      );
      if (reservaRechazada && reservaRechazada.estado === "RECHAZADA") {
        ...
        console.log(
          `⚠️  La reserva tiene estado ${reservaRechazada?.estado}, se esperaba RECHAZADA`,
        );
      }
    }
    console.log("");
    ...
    const solicitudesDespues =
      await obtenerSolicitudesPendientes(federicoToken);
    if (solicitudesDespues && solicitudesDespues.success) {
      console.log(
        `📋 Federico tiene ${solicitudesDespues.data.length} solicitudes pendientes`,
      );
