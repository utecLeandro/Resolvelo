...
async function testMariaPublicacion() {
  ...
  try {
    ...
    const createResponse = await fetch(
      `${BASE_URL}/publicaciones?usuarioId=${perfilData.id}`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    const misPublicacionesResponse = await fetch(
      `${BASE_URL}/publicaciones/mis-publicaciones?usuarioId=${perfilData.id}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
    ...
    if (misPublicacionesResponse.ok) {
      ...
      console.log(
        `✅ María ahora tiene ${misPublicaciones.length} publicaciones:`,
      );
